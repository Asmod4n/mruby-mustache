...
static struct RClass *mustache_module(mrb_state *mrb) { return mrb_module_get_id(mrb, MRB_SYM(Mustache)); }
static struct RClass *mustache_err_class(mrb_state *mrb, mrb_sym name) { return mrb_class_get_under_id(mrb, mustache_module(mrb), name); }
static struct RClass *template_class(mrb_state *mrb) { return mrb_class_get_under_id(mrb, mustache_module(mrb), MRB_SYM(Template)); }
...
/* ===================================================================== */
/* indent state — tracks partial indentation                              */
/* ===================================================================== */

typedef struct {
  const char *bytes;
  mrb_int     len;
  int         pending;   /* 1: emit indent before next byte */
} indent_t;
...
/* Emit raw bytes (from interpolated values). Indent only at start if pending;
 * newlines inside the value do NOT trigger further indent. */
static inline void
...
/* Emit static text bytes (from OP_TEXT). Newlines in the text trigger
 * pending indent for the next emission. */
static void
...
static inline int is_space_byte(unsigned char b) { return b == 32 || b == 9 || b == 10 || b == 13 || b == 11 || b == 12; }
static inline int is_ws_inline(unsigned char b)  { return b == 32 || b == 9 || b == 13; }

static int ws_only(const char *p, mrb_int lo, mrb_int hi) {
  for (mrb_int i = lo; i < hi; i++) if (!is_ws_inline((unsigned char)p[i])) return 0;
  return 1;
}

static mrb_int byteindex_str(const char *hay, mrb_int hlen, const char *needle, mrb_int nlen, mrb_int from) {
  ...
  char first = needle[0];
  while (start < limit) {
    const char *q = (const char*)memchr(start, (unsigned char)first, (size_t)(limit - start));
    if (!q) return -1;
...
static mrb_int byteindex_char(const char *hay, mrb_int hlen, char ch, mrb_int from) {
  if (from < 0) from = 0;
  if (from >= hlen) return -1;
  const char *q = (const char*)memchr(hay + from, (unsigned char)ch, (size_t)(hlen - from));
  return q ? (mrb_int)(q - hay) : -1;
}

/* ===================================================================== */

struct hlk_ctx { const char *name; mrb_int name_len; mrb_value result; int found; };
static int hlk_match(mrb_state *mrb, mrb_value key, mrb_value val, void *data) {
  struct hlk_ctx *c = (struct hlk_ctx*)data;
  mrb_value s = mrb_obj_as_string(mrb, key);
  if (RSTRING_LEN(s) == c->name_len && memcmp(RSTRING_PTR(s), c->name, (size_t)c->name_len) == 0) {
    c->result = val; c->found = 1; return 1;
  }
  return 0;
}
static mrb_value hash_lookup_str(mrb_state *mrb, mrb_value hash, mrb_value name) {
  if (!mrb_hash_p(hash)) return mrb_nil_value();
  struct hlk_ctx c = { RSTRING_PTR(name), RSTRING_LEN(name), mrb_nil_value(), 0 };
  mrb_hash_foreach(mrb, mrb_hash_ptr(hash), hlk_match, &c);
  return c.found ? c.result : mrb_nil_value();
}

static mrb_value ctx_lookup(mrb_state *mrb, mrb_value *stack, mrb_int depth, mrb_value key) {
  mrb_int klen = RARRAY_LEN(key);
  if (klen == 0) return stack[depth - 1];
  mrb_value first = RARRAY_PTR(key)[0];
  ...
  for (mrb_int j = 1; j < klen; j++) {
    ...
    base = hash_lookup_str(mrb, base, RARRAY_PTR(key)[j]);
    if (mrb_nil_p(base)) return mrb_nil_value();
  }
  return base;
}

/* HTML escape: writes via emit_value_bytes (interpolation, no newline indent). */
static void
...
static int section_truthy(mrb_state *mrb, mrb_value v) {
...
static void run_ops(mrb_state *mrb, mrb_value ops, mrb_int pc, mrb_int stop,
                    mrb_value *stack, mrb_int *depth,
                    mrb_value partials, int partial_depth,
                    indent_t *ind, outbuf_t *out);

static void push_or_raise(mrb_state *mrb, mrb_value *stack, mrb_int *depth, mrb_value v) {
  if (*depth >= MUSTACHE_MAX_DEPTH) mrb_raise(mrb, RENDER_ERR(mrb), "max nesting depth exceeded");
  stack[*depth] = v; (*depth)++;
}

static void
render_section(mrb_state *mrb, mrb_value ops, mrb_int pc, mrb_int stop,
               mrb_value *stack, mrb_int *depth,
               mrb_value partials, int partial_depth,
               indent_t *ind, outbuf_t *out, mrb_value v)
{
  if (mrb_array_p(v)) {
    ...
    if (n == 0) return;
    for (mrb_int i = 0; i < n; i++) {
      push_or_raise(mrb, stack, depth, RARRAY_PTR(v)[i]);
               ...
               indent_t *outer_ind, outbuf_t *out)
{
  ...
  /* If outer is already indenting, emit pending now (before the partial starts). */
  if (outer_ind) emit_indent_if_pending(mrb, out, outer_ind);

  if (RSTRING_LEN(indent_str) == 0) {
    /* No indent for this partial — but if outer indent exists, propagate it. */
    run_ops(mrb, sub, 0, RARRAY_LEN(sub), stack, depth,
            partials, partial_depth + 1, outer_ind, out);
    return;
  }

  /* Set up our own indent state. Indent is pending at start of partial. */
  indent_t ind = { RSTRING_PTR(indent_str), RSTRING_LEN(indent_str), 1 };
  run_ops(mrb, sub, 0, RARRAY_LEN(sub), stack, depth,
          partials, partial_depth + 1, &ind, out);
  /* When we return, ind.pending may be true (partial ended on a newline);
   * that's fine — it means no further line in *this* partial. The outer
   * doesn't inherit our pending. */
}

static void
run_ops(mrb_state *mrb, mrb_value ops, mrb_int pc, mrb_int stop,
        mrb_value *stack, mrb_int *depth,
        mrb_value partials, int partial_depth,
        indent_t *ind, outbuf_t *out)
{
  while (pc < stop) {
    mrb_value op = RARRAY_PTR(ops)[pc];
    mrb_int tag = mrb_integer(RARRAY_PTR(op)[0]);

    switch (tag) {
      case OP_TEXT: {
        mrb_value t = RARRAY_PTR(op)[1];
        emit_text_bytes(mrb, out, ind, RSTRING_PTR(t), RSTRING_LEN(t));
        pc++;
        break;
      }
      case OP_VAR: {
        mrb_value v = ctx_lookup(mrb, stack, *depth, RARRAY_PTR(op)[1]);
        if (!mrb_nil_p(v)) {
      ...
      case OP_SECTION: {
        mrb_int end_pc = mrb_integer(RARRAY_PTR(op)[2]);
        mrb_value v = ctx_lookup(mrb, stack, *depth, RARRAY_PTR(op)[1]);
        render_section(mrb, ops, pc + 1, end_pc, stack, depth, partials, partial_depth, ind, out, v);
        pc = end_pc;
        break;
      }
      case OP_INVERTED: {
        ...
        if (!section_truthy(mrb, v))
          run_ops(mrb, ops, pc + 1, end_pc, stack, depth, partials, partial_depth, ind, out);
        pc = end_pc;
        break;
      }
      case OP_PARTIAL:
        render_partial(mrb, RARRAY_PTR(op)[1], RARRAY_PTR(op)[2],
                       stack, depth, partials, partial_depth, ind, out);
        pc++;
        break;
      default:
        mrb_raisef(mrb, RENDER_ERR(mrb), "internal: unexpected op tag %d at pc=%d", (int)tag, (int)pc);
    }
  }
}

/* ===================================================================== */
/* parse_key, strip_name, keys_equal, op constructors — unchanged          */
/* ===================================================================== */

static mrb_value parse_key(mrb_state *mrb, mrb_value src, mrb_int off, mrb_int len) {
  ...
  mrb_int seg_start = off, end = off + len;
...
static mrb_value strip_name(mrb_state *mrb, mrb_value src, const char *p, mrb_int off, mrb_int len, mrb_int err_pos) {
  mrb_int s = off, e = off + len;
  while (s < e && is_space_byte((unsigned char)p[s])) s++;
  while (e > s && is_space_byte((unsigned char)p[e - 1])) e--;
  if (s == e) mrb_raisef(mrb, PARSE_ERR(mrb), "empty partial name at byte %d", (int)err_pos);
  return mrb_str_byte_subseq(mrb, src, s, e - s);
}

static int keys_equal(mrb_value a, mrb_value b) {
  if (mrb_obj_equal(NULL, a, b)) return 1;
  mrb_int la = RARRAY_LEN(a), lb = RARRAY_LEN(b);
  if (la != lb) return 0;
  for (mrb_int i = 0; i < la; i++) {
    mrb_value xa = RARRAY_PTR(a)[i], xb = RARRAY_PTR(b)[i];
    if (!mrb_string_p(xa) || !mrb_string_p(xb)) return 0;
    if (RSTRING_LEN(xa) != RSTRING_LEN(xb)) return 0;
    if (memcmp(RSTRING_PTR(xa), RSTRING_PTR(xb), (size_t)RSTRING_LEN(xa)) != 0) return 0;
  }
  return 1;
}

static void push_op1(mrb_state *mrb, mrb_value ops, mrb_int tag) {
  mrb_value op = mrb_ary_new_capa(mrb, 1);
  mrb_ary_push(mrb, op, mrb_int_value(mrb, tag));
  mrb_ary_push(mrb, ops, op);
}
static void push_op2(mrb_state *mrb, mrb_value ops, mrb_int tag, mrb_value a) {
  mrb_value op = mrb_ary_new_capa(mrb, 2);
  mrb_ary_push(mrb, op, mrb_int_value(mrb, tag));
  mrb_ary_push(mrb, op, a);
  mrb_ary_push(mrb, ops, op);
}
static void push_op3(mrb_state *mrb, mrb_value ops, mrb_int tag, mrb_value a, mrb_value b) {
...
/* ===================================================================== */
/* tokenize, strip_standalone, link_ops — unchanged from previous version */
/* ===================================================================== */
...
tokenize(mrb_state *mrb, mrb_value src)
{
  ...
  const char *p = RSTRING_PTR(src);
  mrb_int sz = RSTRING_LEN(src), pos = 0;
  ...
  while (pos < sz) {
    ...
    if (open_at + 2 < sz && p[open_at + 2] == '{') {
      ...
      if (close_at < 0) mrb_raisef(mrb, PARSE_ERR(mrb), "unclosed {{{ at byte %d", (int)open_at);
      push_op2(mrb, ops, OP_RAW, parse_key(mrb, src, open_at + 3, close_at - open_at - 3));
      pos = close_at + 3;
      continue;
    }
    ...
    if (i == body_len) mrb_raisef(mrb, PARSE_ERR(mrb), "empty tag at byte %d", (int)open_at);
...
static mrb_int op_tag(mrb_value op) { return mrb_integer(RARRAY_PTR(op)[0]); }
static int is_standalone_eligible_tag(mrb_int tag) {
  return tag == OP_SECTION || tag == OP_INVERTED || tag == OP_CLOSE || tag == OP_COMMENT || tag == OP_PARTIAL;
}
...
strip_standalone(mrb_state *mrb, mrb_value ops)
{
  ...
  while (line_start_op < RARRAY_LEN(ops)) {
    ...
    for (mrb_int k = line_start_op; k <= last_op; k++) {
      mrb_value op = RARRAY_PTR(ops)[k];
      mrb_int t = op_tag(op);
      if (t == OP_TEXT) {
        mrb_value tx = RARRAY_PTR(op)[1];
      ...
      if (partial_op_idx >= 0) {
        ...
        if (line_start_op < partial_op_idx) {
          ...
          for (mrb_int k = line_start_op; k < partial_op_idx; k++) {
            mrb_value op = RARRAY_PTR(ops)[k];
            if (op_tag(op) == OP_TEXT) {
              mrb_value tx = RARRAY_PTR(op)[1];
              mrb_int lo = (k == line_start_op) ? line_start_inset : 0;
              total_indent += RSTRING_LEN(tx) - lo;
            }
          }
          if (total_indent > 0) {
        ...
        mrb_value pop = RARRAY_PTR(ops)[partial_op_idx];
    ...
    if (end_op >= 0) {
      if (eligible && has_standalone) {
        ...
        mrb_value t = RARRAY_PTR(RARRAY_PTR(ops)[end_op])[1];
        if (RSTRING_LEN(t) <= line_start_inset) { line_start_op = end_op + 1; line_start_inset = 0; }
      } else {
        line_start_op = end_op;
        line_start_inset = end_inset + 1;
        mrb_value t = RARRAY_PTR(RARRAY_PTR(ops)[end_op])[1];
        if (line_start_inset >= RSTRING_LEN(t)) { line_start_op = end_op + 1; line_start_inset = 0; }
      }
    } else break;
  }
  return ops;
...
static mrb_value key_to_str(mrb_state *mrb, mrb_value k) {
...
link_ops(mrb_state *mrb, mrb_value tokens)
{
  ...
  for (mrb_int i = 0; i < n; i++) {
    mrb_value op = RARRAY_PTR(tokens)[i];
    mrb_int tag = op_tag(op);
    ...
    if (tag == OP_SECTION || tag == OP_INVERTED) {
      ...
      mrb_ary_push(mrb, entry, RARRAY_PTR(op)[1]);
      mrb_ary_push(mrb, stk, entry);
      mrb_ary_push(mrb, out, op);
    } else if (tag == OP_CLOSE) {
      if (RARRAY_LEN(stk) == 0)
        mrb_raisef(mrb, PARSE_ERR(mrb), "closing tag without opener: %S", key_to_str(mrb, RARRAY_PTR(op)[1]));
      mrb_value entry = mrb_ary_pop(mrb, stk);
      mrb_int opener_idx = mrb_integer(RARRAY_PTR(entry)[0]);
      mrb_value opener_key = RARRAY_PTR(entry)[1];
      if (!keys_equal(opener_key, RARRAY_PTR(op)[1]))
        mrb_raisef(mrb, PARSE_ERR(mrb), "mismatched: opened %S, closed %S", key_to_str(mrb, opener_key), key_to_str(mrb, RARRAY_PTR(op)[1]));
      mrb_value opener = RARRAY_PTR(out)[opener_idx];
      mrb_ary_set(mrb, opener, 2, mrb_int_value(mrb, RARRAY_LEN(out)));
    } else if (tag == OP_COMMENT) {
      /* drop */
    } else if (tag == OP_TEXT && RSTRING_LEN(RARRAY_PTR(op)[1]) == 0) {
      /* drop empty */
    } else {
      mrb_ary_push(mrb, out, op);
    }
  ...
  if (RARRAY_LEN(stk) > 0) {
    mrb_value entry = RARRAY_PTR(stk)[RARRAY_LEN(stk) - 1];
    mrb_raisef(mrb, PARSE_ERR(mrb), "unclosed section: %S", key_to_str(mrb, RARRAY_PTR(entry)[1]));
  }
  return out;
}
...
template_tags(mrb_state *mrb, mrb_value self)
{
  ...
  mrb_int n = RARRAY_LEN(ops);
  for (mrb_int i = 0; i < n; i++) {
    mrb_value op = RARRAY_PTR(ops)[i];
    mrb_int t = op_tag(op);
    if (t == OP_VAR || t == OP_RAW || t == OP_SECTION || t == OP_INVERTED) {
      mrb_value label = key_to_str(mrb, RARRAY_PTR(op)[1]);
